...
recon_dir = data_root / "end_to_end2"
...
# Modalità esecuzione
# ---------------------------------------------------------------------------
TRAIN_MODE = input("Vuoi eseguire il training? [y/n]: ").strip().lower() in ["y", "yes", "s", "si"]

print(f"Training abilitato: {TRAIN_MODE}")
...
def visualize_sample(model, dataset, K, device, n_angles, title=''):
    model.eval()
    x_true, y_delta, target, _ = dataset[0]
    ...
    for ax, img, t in zip(
        ...
        ax.axis('off')
    plt.tight_layout()
    plt.savefig(results_dir / f"sample_{n_angles}_{LOSS_NAME}.png", dpi=150, bbox_inches="tight")
    plt.close(fig)
...
if TRAIN_MODE:
    ...
    print("\nTraining saltato. Verranno caricati i pesi già salvati e verrà eseguita solo l'inferenza.")
...
for n_angles in ANGLE_CONFIGS:
    ...
    angle_dir = recon_dir / f"{n_angles}_angles"
    ...
    with torch.no_grad():
        for x_batch, y_delta_batch, target_batch, fnames in test_loader:
            ...
            x_rec_np = x_rec.detach().cpu().numpy().astype(np.float32)

            for i, fname in enumerate(fnames):
                out_path = angle_dir / fname
                np.save(out_path, x_rec_np[i, 0])
