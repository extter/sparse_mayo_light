...
# ---------------------------------------------------------------------------
# Opzioni disponibili per loss_fn in train()
# ---------------------------------------------------------------------------
LOSS_OPTIONS = ['mse', 'ssim', 'ssim_custom', 'mixed']


def get_loss(name: str) -> nn.Module:
    """
    Ritorna la loss corrispondente al nome scelto.

    Opzioni:
        'mse'         -> MSELoss standard
        'ssim'        -> 1 - SSIM (torchmetrics)
        'ssim_custom' -> 1 - SSIM implementazione custom
        'mixed'       -> 0.8*(1-SSIM_custom) + 0.15*L1 + 0.05*MSE
    """
    assert name in LOSS_OPTIONS, f"Loss '{name}' non valida. Scegli tra: {LOSS_OPTIONS}"
    if name == 'mse':
        return MSELoss()
    elif name == 'ssim':
        return SSIMLoss()
    elif name == 'ssim_custom':
        return SSIMCustomLoss()
    elif name == 'mixed':
        return MixedLoss()
...
class SSIMLoss(nn.Module):
    """1 - SSIM usando torchmetrics."""

    def forward(self, pred, target):
        pred = pred.float()
        target = target.float()
        return 1.0 - structural_similarity_index_measure(pred, target, data_range=1.0)


# ---------------------------------------------------------------------------
# SSIM custom (implementazione gaussiana propria)
# ---------------------------------------------------------------------------
class _SSIMModule(nn.Module):
    """Implementazione interna SSIM con finestra gaussiana."""
...
class SSIMCustomLoss(nn.Module):
    """1 - SSIM usando l'implementazione custom con finestra gaussiana."""

    def __init__(self):
        super().__init__()
        self.ssim = _SSIMModule(
            data_range=1.0, window_size=11, sigma=1.5, channels=1, size_average=True
        )
...
# ---------------------------------------------------------------------------
# Mixed loss: SSIM_custom + L1 + MSE
# ---------------------------------------------------------------------------
class MixedLoss(nn.Module):
    """
    Loss mista: w_ssim*(1-SSIM_custom) + w_l1*L1 + w_mse*MSE
    Pesi di default: 0.8 / 0.15 / 0.05
    """
